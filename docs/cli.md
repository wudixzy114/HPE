...
When omitted from the repository-level player commands, `--deck-root` defaults to `presentations/hello-hpe`. The `deck` CLI's separate `--root` option still defaults to `app`, so structural edits do not silently target the presentation shown by the player. A selected deck owns only `deck.json`, `slides/`, `themes` and `assets`; it does not copy or modify the player shell. The compiler and dev server allow source files from that root while keeping manifest, theme, asset and slide validation scoped to it.
